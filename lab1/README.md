# Lab 1 — Minecraft Knowledge Base

## Предметная область

В качестве предметной области выбрана игра **Minecraft**.

База знаний описывает основные объекты игрового мира и связи между ними: мобов, предметы, биомы, измерения, блоки и структуры.

## Сущности

В модели используются следующие сущности:

* `Mob` — моб;
* `Item` — предмет;
* `Biome` — биом;
* `Dimension` — измерение;
* `Block` — блок;
* `Structure` — структура.

## Свойства сущностей

### Mob

Для мобов учитывается их тип поведения:

```prolog
passive(Mob)
aggressive(Mob)
neutral(Mob)
```

### Item

Для предметов учитываются способ получения, тип предмета и размер стака:

```prolog
crafted(Item)
dropped(Item)

is_block(Item)
is_weapon(Item)
is_armor(Item)
is_material(Item)

stack_size(Item, Count)
```

## Отношения между сущностями

### Мобы

```prolog
drops(Mob, Item)
```

Моб может оставить предмет после убийства.

```prolog
spawns(Mob, Biome)
```

Моб может появляться в определённом биоме.

```prolog
uses(Mob, Item)
```

Моб может использовать определённый предмет.

### Биомы и измерения

```prolog
locates(Biome, Dimension)
```

Биом располагается в определённом измерении.

```prolog
biome_major_block(Biome, Block)
```

Определённый блок является преобладающим или характерным для биома.

```prolog
biome_unique_mob(Biome, Mob)
```

Моб является уникальным для определённого биома.

### Структуры

```prolog
generates(Structure, Biome)
```

Структура генерируется в определённом биоме.

```prolog
contains(Structure, Item)
```

Структура может содержать определённые предметы.

```prolog
is_built(Structure, Block)
```

Структура строится из определённых блоков.

### Блоки

```prolog
is_mined(Block, Biome)
```

Блок может быть добыт в определённом биоме.

### Предметы и блоки

```prolog
uses_as(Item, Block)
```

Предмет может использоваться в игровом мире как блок.

### Измерения

```prolog
dimension_major_block(Dimension, Block)
```

Блок является преобладающим для определённого измерения.

```prolog
dimension_unique_item(Dimension, Item)
```

Предмет является уникальным для определённого измерения.

```prolog
dimension_unique_structure(Dimension, Structure)
```

Структура является уникальной для определённого измерения.

```prolog
accessed_by(Dimension, Method)
```

Определяет способ попадания в измерение.

## Выводимые знания

На основе фактов и отношений база знаний должна позволять выводить новые знания.

Планируются следующие типы выводов:

1. Определение измерения, в котором может появляться моб, через биом его появления.
2. Определение измерения, в котором можно встретить блок, через биом его добычи.
3. Определение измерения, в котором может генерироваться структура, через биом её генерации.
4. Определение измерения, в котором можно найти предмет внутри структуры.
5. Определение измерения, в котором можно получить предмет из блока.
6. Определение измерения, в котором можно получить предмет с определённого моба.

Конкретные правила будут реализованы на Prolog после добавления фактов в базу знаний.

## Планируемый объём базы знаний

Модель позволяет реализовать:

* не менее 20 унарных фактов;
* не менее 10–15 фактов-отношений;
* 5–7 выводимых правил.

Конкретные игровые объекты и факты будут добавлены на следующем этапе.
